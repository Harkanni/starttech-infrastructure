...
variable "alb_exists" {
  type        = bool
  default     = true
  description = "Set to true ONLY after the Kubernetes Ingress has successfully created the ALB."
}
...
module "cdn" {
  ...
  # Safe lookup: if count is 0, this passes a blank string instead of throwing an error
  alb_dns_name                         = one(data.aws_lb.backend[*].dns_name)
}
